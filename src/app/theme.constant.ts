import {Theme} from './models/theme.model';

export const xmas: Theme = {
  name: 'xmas',
  properties: {
    '--color_bg_1': '#3e5356',
    '--color_bg_2': '#68a1ab',
    '--color_primary': '#ee7266',
    '--color_secondary': '#24535b',
    '--color_accent': '#b5bbad',
    '--color_day_text': '#ffffff',
    '--color_day_active_bg': '#000000',
    '--color_day_anim_bg1': '#000000',
    '--color_day_anim_bg2': '#1a3b43',
    '--color_day_anim_text': '#ee7266',
    '--color_container_bg': '#ffffff',
    '--color_main_bg': '#000000',
    '--color_main_text': '#ee7266',
    '--color_card_bg': '#ffffff',
    '--color_card_text': '#ee7266',
    '--color_box_bg': '#ffffff',
    '--color_box_text': '#000000',
    '--color_box_src_text': '#000000',
    '--color_box_src_border': '#d3d3d3',
    '--color_box_button_text': '#000000',
    '--color_box_button_bg': '#dce1d5',
    '--color_box_button_bg_active': '#b5bbad',
    '--color_copyright': '#d6d6d6',
    '--color_not_supported_text': '#ffffff',
    '--main_img_filter': 'none'
  }
};

export const jeans: Theme = {
  name: 'jeans',
  properties: {
    '--color_bg_1': '#a7a7a7',
    '--color_bg_2': '#8bc8d3',
    '--color_primary': '#5484a2',
    '--color_secondary': '#c3b57f',
    '--color_accent': '#727272',
    '--color_day_text': '#ffffff',
    '--color_day_active_bg': '#000000',
    '--color_day_anim_bg1': '#000000',
    '--color_day_anim_bg2': '#333333',
    '--color_day_anim_text': '#5484a2',
    '--color_container_bg': '#ffffff',
    '--color_main_bg': '#ffffff',
    '--color_main_text': '#c3b57f',
    '--color_card_bg': '#363f63',
    '--color_card_text': '#7babca',
    '--color_box_bg': '#ffffff',
    '--color_box_text': '#000000',
    '--color_box_src_text': '#000000',
    '--color_box_src_border': '#d3d3d3',
    '--color_box_button_text': '#000000',
    '--color_box_button_bg': '#6994af',
    '--color_box_button_bg_active': '#dce1d5',
    '--color_copyright': '#595959',
    '--color_not_supported_text': '#ffffff',
    '--main_img_filter': 'invert()'
  }
};

export const elegant: Theme = {
  name: 'elegant',
  properties: {
    '--color_bg_1': '#322b25',
    '--color_bg_2': '#335b62',
    '--color_primary': '#2a2a2a',
    '--color_secondary': '#ff7600',
    '--color_accent': '#727272',
    '--color_day_text': '#ffffff',
    '--color_day_active_bg': '#000000',
    '--color_day_anim_bg1': '#2a2a2a',
    '--color_day_anim_bg2': '#535252',
    '--color_day_anim_text': '#ff7600',
    '--color_container_bg': '#ffffff',
    '--color_main_bg': '#000000',
    '--color_main_text': '#ff7600',
    '--color_card_bg': '#bbbbbb',
    '--color_card_text': '#2a2b2e',
    '--color_box_bg': '#ffffff',
    '--color_box_text': '#000000',
    '--color_box_src_text': '#000000',
    '--color_box_src_border': '#d3d3d3',
    '--color_box_button_text': '#ffffff',
    '--color_box_button_bg': '#2a2a2a',
    '--color_box_button_bg_active': '#ff7600',
    '--color_copyright': '#717171',
    '--color_not_supported_text': '#ffffff',
    '--main_img_filter': 'none'
  }
};

export const leaf: Theme = {
  name: 'leaf',
  properties: {
    '--color_bg_1': '#448187',
    '--color_bg_2': '#c8f3b3',
    '--color_primary': '#83a558',
    '--color_secondary': '#98cd7c',
    '--color_accent': '#b0b587',
    '--color_day_text': '#ffffff',
    '--color_day_active_bg': '#c3c605',
    '--color_day_anim_bg1': '#1f3702',
    '--color_day_anim_bg2': '#2e5302',
    '--color_day_anim_text': '#ffffff',
    '--color_container_bg': '#ffffff',
    '--color_main_bg': '#ffffff',
    '--color_main_text': '#767676',
    '--color_card_bg': '#ffffff',
    '--color_card_text': '#767676',
    '--color_box_bg': '#ffffff',
    '--color_box_text': '#000000',
    '--color_box_src_text': '#000000',
    '--color_box_src_border': '#d3d3d3',
    '--color_box_button_text': '#ffffff',
    '--color_box_button_bg': '#83a558',
    '--color_box_button_bg_active': '#bdde9f',
    '--color_copyright': '#d1edc1',
    '--color_not_supported_text': '#ffffff',
    '--main_img_filter': 'invert() opacity(0.6)'
  }
};

export const ocean: Theme = {
  name: 'ocean',
  properties: {
    '--color_bg_1': '#000000',
    '--color_bg_2': '#666666',
    '--color_primary': '#8baebb',
    '--color_secondary': '#94c9dd',
    '--color_accent': '#154a5e',
    '--color_day_text': '#ffffff',
    '--color_day_active_bg': '#000000',
    '--color_day_anim_bg1': '#0e313e',
    '--color_day_anim_bg2': '#154a5e',
    '--color_day_anim_text': '#ffffff',
    '--color_container_bg': '#ffffff',
    '--color_main_bg': '#ffffff',
    '--color_main_text': '#767676',
    '--color_card_bg': '#ffffff',
    '--color_card_text': '#767676',
    '--color_box_bg': '#ffffff',
    '--color_box_text': '#000000',
    '--color_box_src_text': '#000000',
    '--color_box_src_border': '#d3d3d3',
    '--color_box_button_text': '#ffffff',
    '--color_box_button_bg': '#0e313e',
    '--color_box_button_bg_active': '#154a5e',
    '--color_copyright': '#3d7d95',
    '--color_not_supported_text': '#ffffff',
    '--main_img_filter': 'invert() opacity(1)'
  }
};
